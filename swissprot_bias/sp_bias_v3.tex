...
large-scale interrogations of protein functions. Consequently, some papers 
reveal one or more functional aspects of a large amount of proteins
as a result of the particular type of interrogation conducted. To 
...
protein, there is also an \textit{evidence code} which is used to
...
annotation was not made by a curator, and is not checked by a person.
...
was used for annotation. Therefore, the bias we are checking is a per-species
and per-ontology one.  This means that it may be that only a single functional
